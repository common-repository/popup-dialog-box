...
if(!$_POST && isset($_GET['dialoguebox_blink'])&&isset($_GET['dialoguebox_blink'])=='en'){
if (! isset( $_REQUEST['_wpnonce'] ) || ! wp_verify_nonce( $_REQUEST['_wpnonce'],'dialoguebox_blink')){
    wp_nonce_ays( 'dialoguebox_blink');
    exit;
}
...
if(get_option('xyz_credit_link')=="0" &&(get_option('xyz_dbx_credit_dismiss')=="0"))
{
	?>
<div style="float:left;background-color: #FFECB3;border-radius:5px;padding: 0px 5px;border: 1px solid #E0AB1B" id="xyz_dbx_backlink_div">
	
	Please do a favour by enabling backlink to our site. <a class="xyz_dbx_backlink" id="xyz_dbx_backlink" style="cursor: pointer;" >Okay, Enable</a>.
	 <a id="xyz_dbx_dismiss" style="cursor: pointer;" >Dismiss</a>.
	 
<script type="text/javascript">
	 jQuery(document).ready(function() {
		 jQuery('#xyz_dbx_backlink').click(function(){
				xyz_filter_blink_dbx(1)
			});

			jQuery('#xyz_dbx_dismiss').click(function(){
				xyz_filter_blink_dbx(-1)
			});
			
			function xyz_filter_blink_dbx(stat){
				var backlink_nonce= '<?php echo wp_create_nonce('backlink');?>';
				var dataString = { 
						action: 'xyz_dbx_ajax_backlink', 
						enable: stat ,
				...
				jQuery.post(ajaxurl, dataString, function(response) {
					...
					if(response=="dbx"){
						jQuery('.xyz_dbx_backlink').hide();
						jQuery("#xyz_dbx_backlink_div").html('Thank you for enabling backlink !');
						jQuery("#xyz_dbx_backlink_div").css('background-color', '#D8E8DA');
						jQuery("#xyz_dbx_backlink_div").css('border', '1px solid #0F801C');
					}
					if(response==-1){
						jQuery("#xyz_dbx_backlink_div").remove();
				    }
		         });
		  };
		});
...
<td  style="float:right;">
	<a class="xyz_dbx_link" style="margin-left:8px;"  target="_blank" href="http://xyzscripts.com/donate/5"><b>Donate</b></a>
</td>
<td style="float:right;">
